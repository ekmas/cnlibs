...
import { getDitherPaletteSnapshot, rgb, type Seed, subscribeDitherPalette } from "./palette"
...
export function Dot({
  ...
  // Dot renders SVG (not canvas) and is passed in as pre-created `children`
  // from the caller, so a chart-root re-render doesn't reach it — subscribe
  // directly so it re-renders once real colours are ready (see palette.ts).
