...
import { cn } from "./lib"
import { getDitherPaletteSnapshot, rgb, subscribeDitherPalette } from "./palette"
...
export function Legend({
  ...
  // Legend's swatch colours are DOM (not canvas), and Legend itself is
  // passed in as pre-created `children` from the caller — so a chart-root
  // re-render doesn't reach it. Subscribe directly so it re-renders itself
  // once the real (theme-resolved) palette is ready — see palette.ts.
      ...
      {chart.names.map((name) => {
        ...
        return (
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 font-heading text-[11px] text-muted-foreground transition-opacity",
              isClickable &&
