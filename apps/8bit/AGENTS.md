...
- `registry/themes.ts` — the theme registry's single source of truth: one `ThemeRegistryEntry` per theme (`slug`, `title`, `description`, `cssVars: { theme?, light, dark }`), matching shadcn's `registry-item` `cssVars` shape directly. Feeds three things: the live preview/select on `/theme`, the compiled `registry:style` items in `public/r/themes/{slug}.json`, and `registry.json`. Each compiled item's `css` field is generated at build time by parsing the `px-rounded-*`/`px-border-*`/`px-ring` `@utility` rules out of `app/globals.css` (via postcss), so every theme install also ships the pixel-corner utilities — no need to hand-maintain that CSS in two places. Fully self-hosted — no runtime or build-time dependency on an external theme source. Add a theme by appending an entry here, then re-running `pnpm registry:build`.
...
- `lib/` — `utils.ts` (`cn`), `site.ts` (`SITE_URL`, used to build install commands — resolves to the production domain when `NODE_ENV` is `"production"` and to `http://localhost:$PORT` (default 3000) otherwise, so `pnpm dev` shows working `localhost` install commands with no config needed), `docs.ts`, `package-manager.ts` (persisted npm/pnpm/yarn/bun choice), `theme-preset.ts` (persisted `/theme` selection), `use-copy-to-clipboard.ts`.
...
- `pnpm registry:build` (repo root) — regenerate `registry.json` + `public/r/*.json` from `components/ui/*.tsx` and `public/r/themes/*.json` from `registry/themes.ts`. These are committed artifacts, so this always bakes in the production `SITE_URL` (`https://8bit.cnlibs.com`) regardless of the shell's `NODE_ENV`. Use `pnpm registry:build:dev` instead (adds `--dev`) to point the generated `registryDependencies` URLs at `localhost:3000` for testing an in-progress component end to end via `shadcn add`/`init` before it's published — don't commit its output.
- `pnpm registry:test` (repo root) — writes a single scratch `registry:style` item (the "default" theme by default, `registryDependencies` pointed at `localhost:3000/r/button.json`) to `apps/8bit/public/r/test-registry.json` and prints the `npx shadcn add http://localhost:3000/r/test-registry.json` command to try it — the fast path for checking a registry change end to end without regenerating (or accidentally committing) the full theme set. Pass a different theme's colors with `node scripts/generate-registry.mjs --test=<slug>` (e.g. `--test=pastel-dreams`). The output file is gitignored; requires `pnpm dev` running.
